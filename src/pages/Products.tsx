...
export default function Products() {
  ...
  useSEO({ title: 'Charcoal Products', description: 'Premium hardwood, bamboo, and coconut shell charcoal for export from Nigeria. SGS certified, consistent quality.' });
  ...
  useEffect(() => {
    (async () => {
      ...
      setFetchError(false);
      try {
        const { data, error } = await supabase.from('products').select('*').eq('service_type', 'charcoal').order('created_at', { ascending: false });
      ...
      <section className="py-16 md:py-20" style={{ backgroundColor: 'var(--tcf-primary)' }}>
        <div className="max-w-7xl mx-auto px-6 lg:px-8 text-center">
          ...
          <p className="max-w-xl mx-auto mt-4 text-sm leading-relaxed" style={{ color: 'var(--tcf-secondary-text)' }}>
            Hardwood, bamboo, and coconut shell charcoal — sourced from Nigeria and processed to international export standards.
          </p>
        </div>
      </section>
            ...
            <>
              <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
                ...
                <div className="relative md:ml-auto md:w-64">
                  ...
                  <input id="product-search" type="text" value={search}
